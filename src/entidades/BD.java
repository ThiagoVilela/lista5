...
public class BD {
	...
	public StringBuilder gerarBackup(){
		...
		if(alunos.size() > 0){
			...
			//Formatação do dia
			DateFormat dataAluno = DateFormat.getDateInstance();
			
			for (int i = 0; i < alunos.size(); i++) {
				cabecalho.append("\n" + (i+1) + ";" + alunos.get(i).getNome() + ";" + 
						dataAluno.format(alunos.get(i).getDataNascimento()));
			}
		...
		if(professores.size() > 0){
			...
			for (int i = 0; i < professores.size(); i++) {
				cabecalho.append("\n" + professores.get(i).getCpf() + ";" + professores.get(i).getNome() + ";" + 
		...
		if(disciplinas.size() > 0){
			...
			for (int i = 0; i < disciplinas.size(); i++) {
				cabecalho.append("\n" + disciplinas.get(i).getCodigo() + ";" + disciplinas.get(i).getNome() + ";" + 
		...
		if(matriculas.size() > 0){
			...
			for (int i = 0; i < matriculas.size(); i++) {
				cabecalho.append("\n" + matriculas.get(i).getDisciplina().getCodigo() + ";" + 
						matriculas.get(i).getAluno().getMatricula() + ";" + matriculas.get(i).getPontuacao());
			}
		} else{
